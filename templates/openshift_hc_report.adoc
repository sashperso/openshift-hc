...
|===
{% set limit_percent = resource.limits | regex_replace('\(([0-9]{1-3})%\)', '\\1') | int %}
{% if limit_percent >= 90 %}
image::{{ warning_icon }}
{{ node.node_name }} is overcommitted, or close to overcommitted, for the {{ resource.resource_type }} resource.
{% endif %}
